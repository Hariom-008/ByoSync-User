...
extension FaceManager {
    ...
    private func trunc4(_ x: Float) -> Float {
        ...
        func calculateOptionalAndMandatoryDistances() {
            ...
            let iodAtCapture = iodNormalized  // pick one consistently
            ...
            DispatchQueue.main.async { [weak self] in
                ...
                switch self.frameCollectionMode {
                case .registration:
                    self.storeRegistrationFrame(fd, direction: dir)
                case .verification:
                    self.capturedFrames.append(fd)
...
extension FaceManager{
    ...
    func shouldAcceptRegistrationFrame() -> (accept: Bool, direction: HeadDirection?) {
    ...
    func advanceRegistrationPhaseIfNeeded() {
        if case .centerCollecting = registrationPhase, centerFrames.count >= 60 {
            startMovementPhase(durationSec: 15)
        }
    }

    func registrationFramesForUpload() -> [FrameDistance] {
           // Option A (if you keep one array)
           // return capturedFrames.filter { $0.distances.count == 316 }

           // Option B (if you split phases)
