...
final class LoginViewModel: ObservableObject {
    ...
    init(cryptoService: any CryptoService) {
        ...
        Logger.shared.d("AUTH_LOGIN", "LoginViewModel init", user: UserSession.shared.currentUser?.userId)
    }
    ...
    func login() async {
        ...
        guard !trimmed.isEmpty else {
            ...
            Logger.shared.e("AUTH_LOGIN", "Validation failed: empty name", user: UserSession.shared.currentUser?.userId)
            return
        }
        ...
        guard !deviceKey.isEmpty else {
            ...
            Logger.shared.e("AUTH_LOGIN", "DeviceIdentity.resolve() returned empty deviceKey", user: UserSession.shared.currentUser?.userId)
            return
        }
        ...
        #endif
        Logger.shared.i("AUTH_LOGIN", "Login start", user: UserSession.shared.currentUser?.userId)
    ...
    private func updateUserSession(response: APIResponse<LoginData>) {
        guard let userData = response.data?.user,
            ...
            #endif
            Logger.shared.e("AUTH_LOGIN", "Missing user/device in response payload", user: UserSession.shared.currentUser?.userId)
            return
        }
        ...
        #endif
        Logger.shared.d("AUTH_LOGIN", "Decrypting user payload", user: userData.id)
